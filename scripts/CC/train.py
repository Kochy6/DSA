...
# ---------------------------------------------------------------------------
cfg = {
    "csv_path":            r"/root/autodl-tmp/DSA/cleansed_list.csv",  # 改为 AutoDL 的高速本地数据盘
    ...
    "head_weight_decay":   0.01,      # was 0.05; stronger head regularization
...
def train_one_epoch(model, loader, optimizer, criterion, device, epoch, scaler):
    ...
    for batch_idx, (images, labels) in enumerate(loader): # 遍历 DataLoader 的每个 batch
        ...
        if batch_idx == 0:
            with torch.no_grad():
                # 注意：经过 [-1, 1] 归一化后的数据直接保存会有问题，需要反归一化到 [0, 1] 才能正常显示
                img_plot = (
                    images[0, 0, images.shape[2] // 2].cpu().numpy()
                    if images.ndim == 5
                    else images[0, 0].cpu().numpy()
                )
                img_plot = (img_plot + 1.0) / 2.0  # 反归一化 [-1, 1] -> [0, 1]
